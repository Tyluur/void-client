...
final class TextureOpTiledSprite extends TextureOpSprite {
    static int anInt10425;
    static Index LOADING_SCREENS;
    static int anInt10427;

    public TextureOpTiledSprite() {
        /* empty */
    }
    ...
    final int[][] colourOutput(int i, int i_0_) {
        ...
        anInt10425++;
        int[][] is = this.colourCache.get(-93, i);
        if (this.colourCache.dirty && this.method3090(true)) {
            int[] is_1_ = is[0];
            int[] is_2_ = is[1];
            int[] is_3_ = is[2];
    ...
    public static void method3092(int i) {
        if (i != -1) LOADING_SCREENS = null;
        LOADING_SCREENS = null;
    }
