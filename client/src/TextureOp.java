...
// Class348_Sub40
abstract class TextureOp extends Node {
    static int anInt7026;
    static int anInt7027;
    static int anInt7028;
    static int anInt7029;
    static int anInt7030;
    TextureOp[] operations;
    MonochromeImageCache monochromeCache;
    ColourImageCache colourCache;
    ...
    // method3037
    int spriteId(int i) {
        if (i >= -113) monochromeOutput(-125, -85, 60);
        anInt7027++;
        return -1;
    }
    ...
    static final void updateFeatureMask(int i) {
        ...
        TextureOpColourGradient.objectTypeList.method2009(i_0_, (byte) 109);
        Exception_Sub1.itemTypeList.method1942(i_0_, (byte) -125);
        BillboardType.npcTypeList.method2073(-25032, i_0_);
        TextureOpColourFill.gfxTypeList.method2541(119, i_0_);
    ...
    // method3039
    final int[][] outputColour(byte i, int i_1_, int index) {
        anInt7039++;
        int i_3_ = 9 / ((6 - i) / 37);
        if (this.operations[index].monochrome) {
            int[] mono = this.operations[index].monochromeOutput(i_1_, 255);
            int[][] row = new int[3][];
            row[2] = mono;
            row[1] = mono;
            row[0] = mono;
        ...
        return this.operations[index].colourOutput(i_1_, -1564599039);
    }
    ...
    // method3042
    int[] monochromeOutput(int i, int i_53_) {
    ...
    // method3045
    void createImageCache(int width, int height, int i_55_) {
        anInt7043++;
        int size = (i_55_ != (~this.imageCacheCapacity) ? this.imageCacheCapacity : height);
        if (this.monochrome) this.monochromeCache = new MonochromeImageCache(size, height, width);
        else this.colourCache = new ColourImageCache(size, height, width);
    }

    // method3046
    void cacheReset(byte i) {
        anInt7038++;
        if (i > -102) cacheReset((byte) -112);
        if (this.monochrome) {
            this.monochromeCache.reset((byte) 124);
            this.monochromeCache = null;
        } else {
            this.colourCache.reset(6144);
            this.colourCache = null;
        }
    }

    // method3047
    int[][] colourOutput(int i, int i_57_) {
        anInt7040++;
        if (i_57_ != -1564599039) monochromeOutput(-4, -64, 20);
        throw new IllegalStateException("This operation does not have a colour output");
    }

    // method3048
    final int[] monochromeOutput(int y, int i_58_, int index) {
        anInt7034++;
        if (i_58_ != 633706337) this.monochromeCache = null;
        if (!this.operations[index].monochrome) return (this.operations[index].colourOutput(y, -1564599039)[0]);
        return this.operations[index].monochromeOutput(y, i_58_ + -633706082);
    }

    // method3049
    void decode(Packet packet, int i, int i_60_) {
        anInt7028++;
        if (i_60_ != 31015) updateFeatureMask(-16);
    }

    TextureOp(int count, boolean monochrome) {
        this.operations = new TextureOp[count];
