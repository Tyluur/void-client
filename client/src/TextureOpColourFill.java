...
final class TextureOpColourFill extends TextureOp {
    ...
    private TextureOpColourFill(int i) {
        super(0, false);
        method3095(-104, i);
    }

    final int[][] colourOutput(int i, int i_14_) {
        anInt9246++;
        int[][] is = this.colourCache.get(i_14_ ^ 0x5d41e2b0, i);
        if (this.colourCache.dirty) {
            int[] is_15_ = is[0];
            int[] is_16_ = is[1];
            int[] is_17_ = is[2];
            for (int i_18_ = 0; i_18_ < TextureOpPolarDistortion.width; i_18_++) {
    ...
    final void decode(Packet packet, int i, int i_20_) {
        int i_21_ = i;
